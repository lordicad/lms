...
class ProfileTest extends TestCase
{
    ...
    public function test_teacher_can_save_phone_position_school_and_multiple_subjects(): void
    {
        $teacher = User::factory()->teacher()->create();
        ...
        $teacher->refresh();
        $this->assertSame('+60 12-345 6789', $teacher->phone);
    ...
    public function test_reassigning_homeroom_releases_the_previous_class(): void
    {
        $school = School::factory()->create();
        ...
        $this->actingAs($teacher)->patch(route('profile.update'), $this->payload([
            'school_id' => $school->id,
            'homeroom_class_id' => $classB->id,
        ]))->assertRedirect();

        $this->assertNull($classA->fresh()->homeroom_teacher_id);
        $this->assertSame($teacher->id, $classB->fresh()->homeroom_teacher_id);
    }

    public function test_cannot_take_a_class_that_already_has_a_homeroom_teacher(): void
    {
        $school = School::factory()->create();
        $other = User::factory()->teacher()->create();
        $class = SchoolClass::factory()->for($school)->create(['homeroom_teacher_id' => $other->id]);
        $teacher = User::factory()->teacher()->create();
