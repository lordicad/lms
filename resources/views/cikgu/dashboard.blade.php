                ...
                @forelse ($recentLessons as $lesson)
                    <div class="tp-row">
                        ...
                        <div style="display:flex;flex-direction:column;gap:2px;min-width:0;flex:1">
                            ...
                            <span style="font-size:12.5px;color:var(--tp-muted)"><span style="display:inline-block;vertical-align:middle;background:color-mix(in oklab, {{ $lesson->chapter->subject->color ?: '#17907B' }} var(--pill-bw), var(--pill-bb));color:color-mix(in oklab, {{ $lesson->chapter->subject->color ?: '#17907B' }} var(--pill-fw), var(--pill-fb));border-radius:999px;padding:2px 9px;font-weight:800;font-size:11px">{{ $lesson->chapter->subject->name }}</span> · {{ $lesson->chapter->grade->name }} · {{ __('Bab :n', ['n' => $lesson->chapter->number]) }}</span>
                        </div>
