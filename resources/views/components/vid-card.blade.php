...
<a href="{{ route('video.show', $lesson) }}" class="vid-card"
   style="display:flex;flex-direction:column;height:100%;box-sizing:border-box;text-decoration:none;background:var(--wl-surface);border:1px solid var(--wl-line);border-radius:18px;overflow:hidden;box-shadow:0 4px 16px rgba(46,44,80,.04);cursor:pointer">
    ...
    <div style="padding:{{ $showMeta ? '14px 16px' : '13px 15px' }};flex:1 1 auto;display:flex;flex-direction:column;gap:{{ $showMeta ? '4px' : '8px' }}">
        {{-- Always reserve two lines for the title so a one-line and a two-line card end up the
             same height - the grid stretches a whole row to its tallest card otherwise. --}}
        ...
        @if ($showMeta)
            {{-- Duration is already shown on the thumbnail, so it is left off this meta line.
                 The subject is shown as its colour-coded pill (matching the other cards). --}}
            <div style="margin-top:auto;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                <span style="background:{{ $tagBg }};color:{{ $tagColor }};border-radius:999px;padding:3px 10px;font-family:'Geist',sans-serif;font-size:11px;font-weight:800">{{ $subject->displayName() }}</span>
        ...
        @else
            <div style="margin-top:auto;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                ...
                <span style="font-size:12px;font-weight:700;color:var(--wl-muted)">{{ __('Bab :n', ['n' => $lesson->chapter->number]) }}</span>
                @if ($showViews)
                    <span style="display:inline-flex;align-items:center;gap:4px;font-size:12px;font-weight:700;color:var(--wl-muted)"><img src="{{ asset('images/eye.png') }}" alt="" style="width:15px;height:15px;object-fit:contain">{{ $lesson->views_count }}</span>
                @endif
            </div>
        @endif
    </div>
