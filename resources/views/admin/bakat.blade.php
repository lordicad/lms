    ...
    <div style="display:flex;flex-direction:column;gap:48px">
        ...
        <div style="display:flex;flex-direction:column;gap:14px">
            <div style="display:flex;flex-direction:column;gap:2px">
                <h2 style="margin:0;font-family:'Geist',sans-serif;font-size:17px;font-weight:800;color:var(--tp-ink)">🏆 {{ __('Penyumbang Terbaik') }}</h2>
                <span style="font-size:13px;color:var(--tp-muted)">{{ __('Cikgu dengan kandungan diterbitkan paling banyak') }}</span>
            </div>

            @if ($contributors->isEmpty())
                <div class="tp-empty">
                    <span style="font-size:30px">🏆</span>
                    <h3 style="margin:0;font-family:'Geist',sans-serif;font-size:19px;font-weight:800;color:var(--tp-ink)">{{ __('Tiada penyumbang untuk dipaparkan') }}</h3>
                </div>
            @else
                    ...
                    @foreach ($contributors->take(3) as $i => $teacher)
                        ...
                        <div style="background:var(--tp-surface);border:1.5px solid {{ $m['ring'] }};border-radius:20px;padding:{{ $m['pad'] }} 20px 20px;display:flex;flex-direction:column;align-items:center;gap:7px;box-shadow:0 6px 20px var(--tp-line);order:{{ $m['order'] }}">
                            ...
                            <div style="display:flex;gap:16px;margin-top:2px">
                                <span style="font-size:12px;font-weight:700;color:var(--tp-muted)">👁 {{ number_format((int) $teacher->views_sum) }}</span>
                                <span style="font-size:12px;font-weight:700;color:var(--tp-muted)">♥ {{ number_format((int) $teacher->favourites_sum) }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
                        ...
                        @foreach ($contributors->slice(3)->values() as $i => $teacher)
                            ...
                            <div class="tp-tr" style="display:flex;align-items:center;gap:14px;padding:12px 20px;border-bottom:1px solid var(--tp-line)">
                                ...
                                <div style="display:flex;flex-direction:column;gap:1px;min-width:0;flex:1">
                                    <span style="font-family:'Geist',sans-serif;font-weight:800;font-size:14px;color:var(--tp-ink)">{{ $teacher->name }}</span>
                                    <span style="font-size:12px;color:var(--tp-muted)">{{ $subjectLabel($teacher->id, 1) }}</span>
                                </div>
                                <span style="font-size:12.5px;font-weight:700;color:var(--tp-muted);flex-shrink:0">👁 {{ number_format((int) $teacher->views_sum) }}</span>
                                <span style="font-size:12.5px;font-weight:700;color:var(--tp-muted);flex-shrink:0">♥ {{ number_format((int) $teacher->favourites_sum) }}</span>
                ...
                @endif
                <span style="font-size:12.5px;color:var(--tp-muted)">{{ __('Kandungan diterbitkan = video + bahan + kuiz yang diterbitkan.') }}</span>
            @endif
        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:14px;margin-top:24px">
            <div style="display:flex;flex-direction:column;gap:2px">
                <h2 style="margin:0;font-family:'Geist',sans-serif;font-size:17px;font-weight:800;color:var(--tp-ink)">⭐ {{ __('Kandungan Paling Berjaya') }}</h2>
                <span style="font-size:13px;color:var(--tp-muted)">{{ __('Merentas seluruh platform, tanpa tapisan') }}</span>
            </div>
            <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px">
                {{-- Most-viewed video --}}
                <div style="background:var(--tp-surface);border:1px solid var(--tp-line);border-radius:18px;overflow:hidden;box-shadow:0 2px 10px rgba(46,44,80,.04);display:flex;flex-direction:column">
                    ...
                    <div style="padding:18px 20px;display:flex;flex-direction:column;gap:5px">
                        @if ($topVideo)
                            <span style="font-family:'Geist',sans-serif;font-weight:800;font-size:16px;color:var(--tp-ink)">{{ $topVideo->title }}</span>
                            <span style="font-size:12.5px;color:var(--tp-muted)">{{ $topVideo->chapter->subject->displayName() }} · {{ $topVideo->teacher?->name }}</span>
                            <div style="display:flex;gap:24px;margin-top:12px">
                                <div style="display:flex;flex-direction:column">
                                    <span style="font-size:12px;font-weight:700;color:var(--tp-muted)">{{ __('Tontonan') }}</span>
                                    <span style="font-family:'Geist',sans-serif;font-size:22px;font-weight:800;color:var(--tp-ink)">{{ number_format($topVideo->views_count) }}</span>
                                </div>
                                <div style="display:flex;flex-direction:column">
                                    <span style="font-size:12px;font-weight:700;color:var(--tp-muted)">{{ __('Kegemaran') }}</span>
                                    <span style="font-family:'Geist',sans-serif;font-size:22px;font-weight:800;color:var(--tp-ink)">{{ number_format($topVideo->favourites_count) }}</span>
                                </div>
                            </div>
                        @else
                            <span style="font-size:13.5px;color:var(--tp-muted)">{{ __('Belum ada video.') }}</span>
                        @endif
                    </div>
                </div>
                ...
                <div style="background:var(--tp-surface);border:1px solid var(--tp-line);border-radius:18px;overflow:hidden;box-shadow:0 2px 10px rgba(46,44,80,.04);display:flex;flex-direction:column">
                    ...
                    <div style="padding:18px 20px;display:flex;flex-direction:column;gap:5px">
                        @if ($topMaterial)
                            <span style="font-family:'Geist',sans-serif;font-weight:800;font-size:16px;color:var(--tp-ink)">{{ $topMaterial->title }}</span>
                            <span style="font-size:12.5px;color:var(--tp-muted)">{{ $topMaterial->chapter->subject->displayName() }} · {{ $topMaterial->teacher?->name }}</span>
                            <div style="display:flex;gap:24px;margin-top:12px">
                                <div style="display:flex;flex-direction:column">
                                    <span style="font-size:12px;font-weight:700;color:var(--tp-muted)">{{ __('Muat Turun') }}</span>
                                    <span style="font-family:'Geist',sans-serif;font-size:22px;font-weight:800;color:var(--tp-ink)">{{ number_format($topMaterial->download_count) }}</span>
                                </div>
                            </div>
                        @else
                            <span style="font-size:13.5px;color:var(--tp-muted)">{{ __('Belum ada bahan.') }}</span>
                        @endif
                    </div>
                </div>
                {{-- Most-attempted quiz --}}
                <div style="background:var(--tp-surface);border:1px solid var(--tp-line);border-radius:18px;overflow:hidden;box-shadow:0 2px 10px rgba(46,44,80,.04);display:flex;flex-direction:column">
                    ...
                    <div style="padding:18px 20px;display:flex;flex-direction:column;gap:5px">
                        @if ($topQuiz)
                            <span style="font-family:'Geist',sans-serif;font-weight:800;font-size:16px;color:var(--tp-ink)">{{ $topQuiz->title }}</span>
                            <span style="font-size:12.5px;color:var(--tp-muted)">{{ $topQuiz->chapter->subject->displayName() }} · {{ $topQuiz->teacher?->name }}</span>
                            <div style="display:flex;gap:24px;margin-top:12px">
                                <div style="display:flex;flex-direction:column">
                                    <span style="font-size:12px;font-weight:700;color:var(--tp-muted)">{{ __('Percubaan') }}</span>
                                    <span style="font-family:'Geist',sans-serif;font-size:22px;font-weight:800;color:var(--tp-ink)">{{ number_format($topQuiz->attempt_total) }}</span>
                                </div>
                                <div style="display:flex;flex-direction:column">
                                    <span style="font-size:12px;font-weight:700;color:var(--tp-muted)">{{ __('Lulus') }}</span>
                        ...
                        @else
                            <span style="font-size:13.5px;color:var(--tp-muted)">{{ __('Belum ada percubaan kuiz.') }}</span>
                        @endif
                    </div>
                </div>
            </div>
