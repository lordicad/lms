    ...
    <div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px">
        @foreach ($stats as $s)
            <div style="background:var(--tp-surface);border:1px solid var(--tp-line);border-radius:16px;padding:20px 22px;display:flex;flex-direction:column;gap:6px;box-shadow:0 2px 10px rgba(46,44,80,.04)">
                ...
                <span style="font-size:12.5px;font-weight:700;color:var(--tp-muted)">+{{ number_format($s['delta']) }} {{ __('minggu ini') }}</span>
            </div>
        @endforeach
    </div>
    ...
    <div style="display:grid;grid-template-columns:minmax(0,1.5fr) minmax(0,1fr);gap:20px;align-items:start">
        ...
        <div style="background:var(--tp-surface);border:1px solid var(--tp-line);border-radius:18px;box-shadow:0 2px 10px rgba(46,44,80,.04);overflow:hidden">
            <div style="display:flex;align-items:center;gap:12px;padding:18px 22px;border-bottom:1px solid var(--tp-line)">
                <h2 style="margin:0;font-family:'Geist',sans-serif;font-size:17px;font-weight:800;color:var(--tp-ink);flex:1">{{ __('Pendaftaran Terkini') }}</h2>
            ...
            @forelse ($recentUsers as $i => $u)
                ...
                <div class="tp-tr" style="display:flex;align-items:center;gap:14px;padding:13px 22px;border-bottom:1px solid var(--tp-line)">
            ...
            @empty
                <div style="padding:28px 22px;font-size:13.5px;color:var(--tp-muted)">{{ __('Belum ada pendaftaran.') }}</div>
            @endforelse
        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:20px;min-width:0">
            {{-- Platform activity --}}
            <div style="background:var(--tp-surface);border:1px solid var(--tp-line);border-radius:18px;padding:22px;box-shadow:0 2px 10px rgba(46,44,80,.04);display:flex;flex-direction:column;gap:16px">
                <h2 style="margin:0;font-family:'Geist',sans-serif;font-size:16px;font-weight:800;color:var(--tp-ink)">📊 {{ __('Aktiviti Platform') }}</h2>
                @foreach ($activity as $a)
                    <div style="display:flex;flex-direction:column;gap:6px">
                        <div style="display:flex;align-items:center;justify-content:space-between">
                            <span style="font-size:13px;font-weight:700;color:var(--tp-muted-2)">{{ $a['label'] }}</span>
                            <span style="font-family:'Geist',sans-serif;font-weight:800;font-size:13.5px;color:var(--tp-ink)">{{ number_format($a['value']) }}</span>
                        </div>
                        <div style="height:8px;border-radius:999px;background:var(--tp-line);overflow:hidden">
            ...
            {{-- Pending actions --}}
            <div style="background:var(--tp-surface);border:1px solid var(--tp-line);border-radius:18px;padding:22px;box-shadow:0 2px 10px rgba(46,44,80,.04);display:flex;flex-direction:column;gap:14px">
                <h2 style="margin:0;font-family:'Geist',sans-serif;font-size:16px;font-weight:800;color:var(--tp-ink)">⏳ {{ __('Tindakan Menunggu') }}</h2>
