<x-admin-layout :title="__('Notifikasi')"
                :heading="__('Notifikasi')"
                heading-icon="bell"
                :sub="__('Permintaan dan makluman yang perlu tindakan admin')">
    ...
    @if ($notifications->isEmpty())
        <div class="tp-empty">
            ...
            <p style="margin:0;font-size:14.5px;color:var(--tp-muted);max-width:420px">{{ __('Apabila pengguna memohon set semula kata laluan, ia akan muncul di sini untuk anda tetapkan semula.') }}</p>
        </div>
    @else
            ...
            <div style="background:var(--tp-surface);border:1px solid var(--tp-line);border-radius:18px;overflow:hidden;box-shadow:0 2px 10px rgba(46,44,80,.04)">
                @foreach ($notifications as $n)
                    ...
                    <div style="display:flex;align-items:center;gap:14px;padding:14px 20px;border-bottom:1px solid var(--tp-line);{{ $n->read_at ? '' : 'background:var(--tp-surface-2)' }}">
                        ...
                        @if ($n->url)
                            <a href="{{ $n->url }}" class="tp-btn-outline tp-btn-sm" style="flex-shrink:0">{{ __('Tetapkan semula') }}</a>
                        @endif
                    </div>
                @endforeach
            </div>
