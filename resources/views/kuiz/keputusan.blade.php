<x-student-layout :title="__('Keputusan')">
    ...
    <div style="display:flex;flex-direction:column;gap:24px;max-width:760px;margin:0 auto;width:100%">
        {{-- Score card --}}
        <div style="background:var(--wl-surface);border:1px solid var(--wl-line);border-radius:22px;padding:32px;display:flex;flex-direction:column;align-items:center;gap:14px;text-align:center;box-shadow:0 8px 24px var(--wl-line)">
            ...
            <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;width:100%;margin-top:10px">
                <div style="background:#F6F3EC;border-radius:14px;padding:14px 18px;display:flex;flex-direction:column;gap:3px;text-align:left">
                    <span style="font-size:12.5px;font-weight:700;color:var(--wl-muted)">{{ __('Betul') }}</span>
                    <span style="font-family:'Geist',sans-serif;font-size:20px;font-weight:800;color:var(--wl-ink)">{{ $attempt->correct_count }}/{{ $attempt->question_count }}</span>
                </div>
                <div style="background:#F6F3EC;border-radius:14px;padding:14px 18px;display:flex;flex-direction:column;gap:3px;text-align:left">
                    <span style="font-size:12.5px;font-weight:700;color:var(--wl-muted)">{{ __('Ketepatan') }}</span>
                    <span style="font-family:'Geist',sans-serif;font-size:20px;font-weight:800;color:var(--wl-ink)">{{ $pct }}%</span>
                </div>
                <div style="background:#F6F3EC;border-radius:14px;padding:14px 18px;display:flex;flex-direction:column;gap:3px;text-align:left">
                    <span style="font-size:12.5px;font-weight:700;color:var(--wl-muted)">{{ __('Masa') }}</span>
                    <span style="font-family:'Geist',sans-serif;font-size:20px;font-weight:800;color:var(--wl-ink)">{{ $attempt->humanDuration() }}</span>
                </div>
            </div>
